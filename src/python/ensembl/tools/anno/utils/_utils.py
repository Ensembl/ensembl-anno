                    ...
                    if len(values) == 9 and (values[2] in feature_types) and unique_ids:
                        ...
                        if match_gene_type:
                            ...
                            if transcript_id_slice not in gene_transcript_id_collection:
                                ...
                                gene_transcript_id_collection[transcript_id_slice] = new_transcript_id
                                transcript_id_count_gene_id[gene_id_slice] += 1
                            else:
                            ...
                            if new_id_prefix == "repeatmask":
                                match_feature_type = re.search(
                                    r"(" + feature_id_label + r"\d+)",
                                    line,
                                )
                            else:
                                match_feature_type = (
                                    re.search(  # repeat_id\d+  r"(" + feature_id_label + ' +"([^"]+)")',
...
def get_sequence(
    ...
    with tempfile.NamedTemporaryFile(mode="w+t", delete=False, dir=str(output_dir)) as bed_temp_file:
    ...
    for idx, line in enumerate(io.TextIOWrapper(bedtools_output.stdout, encoding="utf-8")):  # type: ignore
        if idx == 1:
            if strand == 1:
...
def read_gtf_genes(gtf_file: Path) -> Dict[str, Dict[str, Dict[str, List[str]]]]:
    """
    Parses a GTF file to extract gene, transcript, and exon information.

    Args:
        gtf_file: Path to the GTF file.

    Returns:
        A dictionary containing gene information, organized by gene_id, transcript_id, and features.
    """
    gtf_genes: Dict[str, Dict[str, Dict[str, List[str]]]] = {}
    ...
    with open(gtf_file, "r") as gtf_in:
        for line in gtf_in:
            ...
            match = re.search(r'gene_id "([^"]+)".+transcript_id "([^"]+)"', eles[8])
            if not match:
                continue
            ...
            if feature_type == "transcript":
                gtf_genes[gene_id][transcript_id] = {"transcript": [line.strip()], "exons": []}
            elif feature_type == "exon":
                if transcript_id in gtf_genes[gene_id]:
                    gtf_genes[gene_id][transcript_id]["exons"].append(line.strip())

    return gtf_genes


def check_transcriptomic_output(main_output_dir: Path) -> None:
    """Check across the various transcriptomic if there is a reasonable amount of data

    Args:
        main_output_dir (Path): Output dir path
    Raises:
        IOError: If no lines or fewer than the minimum expected lines are found in the output files.
    """
    ...
    for transcriptomic_dir in transcriptomic_dirs:
        ...
        if not full_file_path.exists():
            logger.warning(
                "Warning, no annotation.gtf found for %s. This might be fine, \
                    e.g. no long read data were provided",
                transcriptomic_dir,
            )
            continue
        num_lines = sum(1 for _ in full_file_path.open("r"))
        total_lines += num_lines
        logging.info("For %s found a total of in the annotation.gtf file", num_lines)
    if total_lines == 0:
        raise IOError(
...
#                         orf_out.write(
#                             ">" + region_name + "_phase" + str(phase) + "_orf" + str(orf_counter) + "\n"
#                         )
...
#         else:
#             logger.info("Found an existing split file, so will not overwrite. File found:")
