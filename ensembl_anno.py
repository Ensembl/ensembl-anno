...
def main() -> None:  # pylint: disable=too-many-locals, too-many-branches, too-many-statements
    ...
    red_path = optional_path(args.red_path) | config["red"]["software"]
    geneblast_path = optional_path(args.genblast_path) | config["genblast"]["software"]
    convert2blastmask_path = (
        optional_path(args.convert2blastmask_path) | config["convert2blastmask"]["software"]
    )
    makeblastdb_path = optional_path(args.makeblastdb_path) | config["makeblastdb"]["software"]
    ...
    star_path = optional_path(args.star_path) | config["star"]["software"]
    short_read_fastq_dir = optional_path(args.short_read_fastq_dir)
    max_intron_length = args.max_intron_length
    max_reads_per_sample = args.max_reads_per_sample
    minimap2_path = optional_path(args.minimap2_path) | config["minimap2"]["software"]
    paftools_path = optional_path(args.paftools_path) | config["paftools"]["software"]
    ...
    augustus_path = optional_path(args.augustus_path) | config["augustus"]["software"]
    stringtie_path = optional_path(args.stringtie_path) | config["stringtie"]["software"]
    scallop_path = optional_path(args.scallop_path) | config["scallop"]["software"]
    subsample_script_path = optional_path(args.subsample_script_path)
    samtools_path = optional_path(args.samtools_path) | config["samtools"]["software"]
    db_details = args.db_details
    trf_path = optional_path(args.trf_path) | config["trf"]["software"]
    dust_path = optional_path(args.dust_path) | config["dust"]["software"]
    trnascan_path = optional_path(args.trnascan_path) | config["trnascan"]["software"]
    trnascan_filter_path = optional_path(args.trnascan_filter_path) | config["trnascan"]["filter_path"]
    cpg_path = optional_path(args.cpg_path) | config["cpg"]["software"]
    eponine_path = optional_path(args.eponine_path) | config["eponine"]["software"]
    java_path = optional_path(args.java_path) | config["java"]["path"]
    repeatmasker_path = optional_path(args.repeatmasker_path) | config["repeatmasker"]["software"]
    ...
    trim_galore_path = optional_path(args.trim_galore_path) | config["trim_galore"]["software"]
