...
def check_transcriptomic_output(args):
    ...
    if total_lines == 0:
        raise OSError("Transcriptomic mode was enabled, but all transcriptomic annotation files are empty.")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Minimum total number of gtf lines expected across all tools",
    )
    parser.add_argument(
        "--checks_passed_file",
        help="Path to write file to if all checks pass. Useful to ensure nextflow behaves appropriately.",
    )
