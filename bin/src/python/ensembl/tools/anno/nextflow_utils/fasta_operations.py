...
def write_fasta_intervals_to_bed(args):
    seq_region_to_length_dict = get_seq_region_length(args.genome_file, args.min_seq_length)
    slice_ids = get_slice_id(
        seq_region_to_length_dict,
        slice_size=args.slice_size,
        overlap=args.overlap,
        min_length=args.min_seq_length,
    )  # nb. pass different value on cmdline
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Arguments for script to calculate index base value to pass to STAR in genomeGenerate mode"
    )
    parser.add_argument("--genome_file", help="Genome file path")
    parser.add_argument(
        "--calculate_genomeSAindexNbases", action="store_true", help="calculate star index param"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="maximum chromosome or slice size to consider when calculating genome size",
    )
    ...
    parser.add_argument(
        "--maximum_index_bases", required=False, type=int, default=14, help="maximum index bases value"
    )
