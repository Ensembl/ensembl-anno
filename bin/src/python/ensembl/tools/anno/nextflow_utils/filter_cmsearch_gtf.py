...
def extract_id_from_gtf_line(line):
    ...
    if len(repeat_id_field) != 1:
        raise ValueError(f"Expected one repeat_id but got a different number. \
...
def parse_args():
    parser = argparse.ArgumentParser(description="Arguments for script to filter cmsearch hits not validated by RNAfold")
...
if __name__ == "__main__":
    args = parse_args()
    filter_gtf(input_gtf = args.input_gtf, 
               output_gtf = args.output_gtf,
               rnafold_predictions = args.rnafold_predictions)