...
from gff3_validate import validate_and_fix_gff3
from dedup_genes import dedup_genes
import numpy as np
...
def compute_percentile_guardrails(locus_df, config, protein_supported_tids):
    ...
    if val_cfg.max_exon_len_mode != "percentile" and val_cfg.max_transcript_span_mode != "percentile":
        return runtime_params
        
    if val_cfg.max_exon_len_reference == "reference":
        print("Warning: percentile reference mode 'reference' is not fully supported yet. Falling back to candidates_supported.")
    ...
    for (source, tid), grp in locus_df.groupby(['Source', 'transcript_id']):
        ...
        combined_ev = grp['combined_evidence'].iloc[0] if 'combined_evidence' in grp.columns else source
        if len(combined_ev.split(',')) >= 2:
            high_conf_tids.add(tid)
            
    if not high_conf_tids:
        print("Note: No high-confidence candidates found for percentile calculation. Using fixed limits.")
        return runtime_params
    ...
    if val_cfg.max_transcript_span_mode == 'percentile':
        ...
        if spans:
            ...
            runtime_params['effective_max_transcript_span_bp'] = max(val_cfg.max_transcript_span_bp, calc_limit)
...
def parse_args():
    ...
    parser.add_setup_args.add_argument('--check-deps', action='store_true', 
...
def main():
    ...
    annotations = annotate_all_transcripts(candidate_exons, genome_dict, candidate_cds, min_codons=config.orf.min_codons)
    ...
    if validation_scores:
        candidate_exons['protein_coding_score'] = candidate_exons['transcript_id'].map(validation_scores).fillna(0.0)
    ...
    for cid, locus_df in cluster_df.groupby('Cluster'):
        ...
        for gene_models in genes:
            ...
            for i, model in enumerate(gene_models):
                ...
                utr_support = {
                    'supported_5p': True, 'supported_3p': True,
                    'action_5p': 'kept', 'action_3p': 'kept',
                    'reason_5p': 'default', 'reason_3p': 'default'
                }
                ...
                if not utr_support['supported_5p'] and utr_support['action_5p'] == 'dropped':
                    if ann: ann['five_prime_utr'] = []
                elif not utr_support['supported_5p'] and utr_support['action_5p'] == 'drop_transcript':
                    drop_whole_transcript = True
                    
                # Check 3p
                if not utr_support['supported_3p'] and utr_support['action_3p'] == 'dropped':
                    if ann: ann['three_prime_utr'] = []
                elif not utr_support['supported_3p'] and utr_support['action_3p'] == 'drop_transcript':
                    drop_whole_transcript = True
                ...
                mrna_row = {
                    'Chromosome': gene_chrom, 'Source': 'GMB', 'Feature': 'mRNA',
                    'Start': mrna_start, 'End': mrna_end, 'Score': '.', 'Strand': gene_strand,
                    'Frame': '.', 'ID': new_tid, 'Parent': gene_id,
                ...
                for j, (ex_s, ex_e) in enumerate(exons_sorted):
                    selected_gff_rows.append({
                        'Chromosome': gene_chrom, 'Source': 'GMB', 'Feature': 'exon',
                        'Start': ex_s, 'End': ex_e, 'Score': '.', 'Strand': gene_strand,
                        'Frame': '.', 'ID': f"{new_tid}.exon{j+1}", 'Parent': new_tid
                    })
                    
                if ann:
                    for j, (s, e) in enumerate(ann['cds']):
                        selected_gff_rows.append({
                            'Chromosome': gene_chrom, 'Source': 'GMB', 'Feature': 'CDS',
                            'Start': s, 'End': e, 'Score': '.', 'Strand': gene_strand,
                            'Frame': '.', 'ID': f"{new_tid}.cds{j+1}", 'Parent': new_tid
                        })
                    for j, (s, e) in enumerate(ann['five_prime_utr']):
                        selected_gff_rows.append({
                            'Chromosome': gene_chrom, 'Source': 'GMB', 'Feature': 'five_prime_UTR',
                            'Start': s, 'End': e, 'Score': '.', 'Strand': gene_strand,
                            'Frame': '.', 'ID': f"{new_tid}.5utr{j+1}", 'Parent': new_tid
                        })
                    for j, (s, e) in enumerate(ann['three_prime_utr']):
                        selected_gff_rows.append({
                            'Chromosome': gene_chrom, 'Source': 'GMB', 'Feature': 'three_prime_UTR',
                            'Start': s, 'End': e, 'Score': '.', 'Strand': gene_strand,
                            'Frame': '.', 'ID': f"{new_tid}.3utr{j+1}", 'Parent': new_tid
                        })
            ...
            selected_gff_rows.insert(gene_insert_idx, {
                'Chromosome': gene_chrom, 'Source': 'GMB', 'Feature': 'gene',
                'Start': gene_start, 'End': gene_end, 'Score': '.', 'Strand': gene_strand,
    ...
    if val_stats['violations_found'] > 0:
        print(f"    {val_stats['violations_found']} violations found, "
    ...
    if dedup_stats.get('genes_merged', 0) + dedup_stats.get('genes_dropped', 0) > 0:
        print(f"    {dedup_stats.get('genes_merged', 0)} merged, "
    ...
    for m in output_mrnas:
        ...
        transcript_span_bp = sorted_exons[-1]['End'] - sorted_exons[0]['Start'] if sorted_exons else 0
    ...
    summary_out = {
        ...
        'filtering': stats,
        'utr': {
            'transcripts_dropped': stats.get('utr_drop_transcripts', 0)
        }
    }
