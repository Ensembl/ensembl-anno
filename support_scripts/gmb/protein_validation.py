...
def check_dependencies(val_cfg):
    ...
    try:
        subprocess.run([val_cfg.diamond_path, "help"], 
                       check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:
        print(f"ERROR: Protein validation enabled but diamond binary '{val_cfg.diamond_path}' not found in PATH.", file=sys.stderr)
        sys.exit(1)
        
    try:
        subprocess.run([val_cfg.psauron_path, "--help"], 
                       check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:
        print(f"ERROR: Protein validation enabled but psauron binary '{val_cfg.psauron_path}' not found in PATH.", file=sys.stderr)
        sys.exit(1)
...
def batch_score_proteins(protein_dict, config):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        cmd_d = [
            val_cfg.diamond_path, "blastp",
            "-d", val_cfg.diamond_db,
            "-q", fasta_path,
            "-o", diamond_out,
        ...
        for seq, keys in seq_to_keys.items():
             ...
             comb_score = (d_score * val_cfg.diamond_weight) + \
                          (p_score * val_cfg.psauron_weight)
             for k in keys:
