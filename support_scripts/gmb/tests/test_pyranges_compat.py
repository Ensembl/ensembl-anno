...
class TestPyRangesRoundTrip:
    ...
    def test_overlap_operation(self):
        ...
        df_a = pd.DataFrame({
            'Chromosome': ['1'], 'Start': [100], 'End': [300], 'Strand': ['+'],
        })
        df_b = pd.DataFrame({
            'Chromosome': ['1'], 'Start': [200], 'End': [400], 'Strand': ['+'],
        })
    ...
    def test_no_overlap_returns_empty(self):
        ...
        df_a = pd.DataFrame({
            'Chromosome': ['1'], 'Start': [100], 'End': [200], 'Strand': ['+'],
        })
        df_b = pd.DataFrame({
            'Chromosome': ['1'], 'Start': [500], 'End': [600], 'Strand': ['+'],
        })
        gr_a = pr.PyRanges(df_a)
        gr_b = pr.PyRanges(df_b)
        ovl = gr_a.overlap(gr_b)
