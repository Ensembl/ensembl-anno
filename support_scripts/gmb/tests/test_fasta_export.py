...
class TestExportFasta:
    ...
    def test_reverse_strand_cdna(self, tmp_path, config):
        ...
        model = {
            'id': 'tx_rev', 'source': 'Scallop', 'chrom': 'chr1',
            'strand': '-', 'df': exons_df, 'start': 0, 'end': seq_len,
            'exon_count': 1, 'combined_evidence': 'Scallop',
            'output_tid': 'GENE_R.1', 'gene_id': 'GENE_R',
        }
        loci = [(1, [model])]
        out_dir = str(tmp_path / 'output')
...
class TestHelixerCdsExport:
    def test_helixer_cds_used(self, tmp_path, config):
        ...
        model = {
            'id': 'hx_tx1', 'source': 'Helixer', 'chrom': 'chr1',
            'strand': '+', 'df': exons_df, 'start': 0, 'end': 213,
            'exon_count': 1, 'combined_evidence': 'Helixer',
            'output_tid': 'GENE_H.1', 'gene_id': 'GENE_H',
        }
        loci = [(1, [model])]
        out_dir = str(tmp_path / 'output')
