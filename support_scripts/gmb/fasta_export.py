...
def export_fasta(loci, genome, helixer_cds, config, output_dir):
    ...
    try:
        for cluster_id, models in loci:
            ...
            for i, m in enumerate(models):
                ...
                exons = sorted(
                    zip(m['df']['Start'].values, m['df']['End'].values))
                ...
                if existing_cds:
                    ...
                    if orf is not None:
                        ...
                        cds_intervals = map_cds_to_genomic(
                            cds_start, cds_end, exons, strand)
                ...
                if cds_intervals:
                    ...
                    cds_nuc = _build_cds_seq_from_intervals(
                        cds_intervals, strand, chrom_seq)
                    ...
                    # Write CDS FASTA
                    if cds_fh:
                        cds_header = (f">{tid} gene={gene_id} "
                                      f"cds_len={len(cds_nuc)}"
                                      f"{orf_status}")
                    ...
                    if prot_fh and protein:
                        if not ecfg.include_partial and (
                                is_partial_5 or is_partial_3):
                            continue
                        prot_header = (f">{tid} gene={gene_id} "
                                       f"aa_len={len(protein)}"
                                       f"{orf_status}")
    ...
    print(f"  FASTA export: {stats['total_transcripts']} transcripts, "
