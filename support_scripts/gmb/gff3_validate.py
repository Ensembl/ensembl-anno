...
def _interval_in_union(union_intervals: List[Tuple[int, int]], target_s: int, target_e: int) -> bool:
...
def validate_transcript(mrna_row, exon_rows, cds_rows, utr_rows, config=None, runtime_params=None):
    ...
    eff_max_exon_len = config.max_exon_len_bp if config and hasattr(config, 'max_exon_len_bp') else 15000
    eff_max_span = config.max_transcript_span_bp if config and hasattr(config, 'max_transcript_span_bp') else 100000
    ...
    if max_exon_len > eff_max_exon_len:
        violations.append(f"Merged exon length {max_exon_len} exceeds maximum allowed ({eff_max_exon_len} bp)")

    transcript_span = 0
    if exon_union:
        transcript_span = exon_max - exon_min
        if config and hasattr(config, 'max_transcript_span_mode') and config.max_transcript_span_mode != 'off':
            if transcript_span > eff_max_span:
                violations.append(f"Transcript span {transcript_span} exceeds maximum allowed ({eff_max_span} bp)")
    ...
    if all_children:
        ...
        if mrna_s > child_min:
            violations.append(
                f"mRNA start {mrna_s} > child min {child_min}")
        if mrna_e < child_max:
            violations.append(
                f"mRNA end {mrna_e} < child max {child_max}")

    # CDS within single merged exon segment
    for r in cds_rows:
        if not _interval_in_union(exon_union, r['Start'], r['End']):
            violations.append(
                f"CDS {r['Start']}-{r['End']} not within any single merged exon")

    # UTR within single merged exon segment
    for r in utr_rows:
        if not _interval_in_union(exon_union, r['Start'], r['End']):
            violations.append(
                f"UTR {r['Start']}-{r['End']} not within any single merged exon")
...
def trim_utrs(utr_5p_rows, utr_3p_rows, cds_rows, exon_rows, config):
    ...
    if config.trim_policy == 'drop_utrs':
        if (utr_5p_bp > config.max_5p_bp or utr_3p_bp > config.max_3p_bp or
                utr_5p_bp + utr_3p_bp > config.max_total_bp or
                (cds_bp > 0 and (utr_5p_bp + utr_3p_bp) / cds_bp > config.max_utr_to_cds_ratio)):
            return [], [], True
    ...
    if cds_bp > 0 and (_total_bp(out_5p) + _total_bp(out_3p)) / cds_bp > config.max_utr_to_cds_ratio:
        max_utr = int(cds_bp * config.max_utr_to_cds_ratio)
        ratio = max_utr / (_total_bp(out_5p) + _total_bp(out_3p)) if (_total_bp(out_5p) + _total_bp(out_3p)) > 0 else 0
        out_5p = _hard_cap(out_5p, int(_total_bp(out_5p) * ratio))
        out_3p = _hard_cap(out_3p, int(_total_bp(out_3p) * ratio))
        was_trimmed = True
...
def validate_and_fix_gff3(gff_rows, config, runtime_params=None):
    ...
    for gene_row in genes:
        ...
        for mrna_row in mrna_rows:
            ...
            if policy == 'trim':
                ...
                for r in cds_rows:
                    ...
                    if len(intersections) < 1: stats['containment_failures'] += 1
            ...
            violations, max_drift, _ = validate_transcript(mrna_row, exon_rows, cds_rows, utr_rows, config=val_cfg, runtime_params=runtime_params)

            local_mode = mode
            if hasattr(val_cfg, 'max_feature_drift_bp') and max_drift > val_cfg.max_feature_drift_bp:
            ...
            if violations:
                ...
                if local_mode == 'error' or (policy == 'error' and any("not within any single merged exon" in v for v in violations)):
                    ...
                    verify_violations, _, _ = validate_transcript(mrna_row, exon_rows, cds_rows, utr_rows, config=val_cfg, runtime_params=runtime_params)
                    if verify_violations:
                        ...
                        if policy == 'drop':
                            ...
                            sys.exit(f"Failed to fix validation violations for {tid}: {'; '.join(verify_violations)}")
        ...
        if valid_mrna_rows:
            ...
            output_rows.insert(
                output_rows.index(valid_mrna_rows[0]),
                gene_row)
