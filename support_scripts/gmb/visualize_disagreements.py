...
def _cached_annotate_for_vis(exon_rows, strand, chrom, genome,
                             cds_rows=None, infer_cds=False,
                             min_orf_aa=100):
    ...
    if cds_rows:
        return _annotate_for_vis(
            exon_rows, strand, chrom, genome,
            cds_rows=cds_rows, infer_cds=infer_cds,
...
def _annotate_for_vis(exon_rows, strand, chrom, genome, cds_rows=None,
                      infer_cds=False, min_orf_aa=100):
    ...
    empty = {
        ...
        'start_pos': None, 'stop_pos': None,
        'is_partial_5': False, 'is_partial_3': False,
        'splice_sites': [], 'frame_ok': True, 'orf_label': '',
    }
    ...
    if cds_rows:
        ...
        return {
            ...
            'start_pos': start_pos, 'stop_pos': stop_pos,
            'is_partial_5': False, 'is_partial_3': False,
            'splice_sites': splice, 'frame_ok': frame_ok,
            'orf_label': orf_label,
        }
    ...
    return {
        ...
        'three_prime_utr': three_utr,
        'start_pos': start_pos, 'stop_pos': stop_pos,
        'is_partial_5': is_partial_5, 'is_partial_3': is_partial_3,
        'splice_sites': splice, 'frame_ok': frame_ok,
        'orf_label': orf_label,
    }
        ...
        for _, (rid, cid) in pairs.iterrows():
            ...
            if len(r_rows) != len(c_rows):
                ...
                structural_mismatches.append(
                    (chrom, start, end, "Structural_Mismatch",
                     f"{rid}_vs_{cid}"))
                continue
            ...
            if r_strand != c_strand and c_strand != '.':
                ...
                structural_mismatches.append(
                    (chrom, start, end, "Strand_Mismatch",
                     f"{rid}_vs_{cid}"))

    print(f"  Found {len(structural_mismatches)} Structural Mismatches "
          "to visualize.")
...
def plot_locus(data, chrom, start, end, label, out_filename,
               cds_data=None, genome=None,
               show_utrs=False, show_start_stop=False,
               infer_cds=False, min_orf_aa=100,
               show_splice_qc=False, show_orf_labels=False,
               show_frame_qc=False,
    ...
    for name, pr_obj in data.items():
        ...
        if len(all_tids) > max_tx_per_track:
            ...
            spans = df.groupby('transcript_id').agg(
                s=('Start', 'min'), e=('End', 'max'))
        ...
        for tid, group in df.groupby('transcript_id'):
            ...
            needs_annotation = (show_utrs or show_start_stop or infer_cds
                                or show_splice_qc or show_orf_labels
                                or show_frame_qc)
            ...
            if show_start_stop:
                if ann['start_pos'] is not None:
                    ...
                    if plot_start <= sp <= plot_end:
                        ax.plot(sp, y + CDS_HEIGHT / 2 + 0.04, marker='v',
                                markersize=MARKER_SIZE, color='#2ECC40',
                                zorder=5, clip_on=True)
                elif ann['is_partial_5']:
                    # Hollow marker at transcript edge for partial
                    edge = (exon_list[-1][1] if strand == '-'
                            else exon_list[0][0])
                    if plot_start <= edge <= plot_end:
                        ax.plot(edge, y + CDS_HEIGHT / 2 + 0.04, marker='v',
                                markersize=MARKER_SIZE, color='#2ECC40',
                ...
                if ann['stop_pos'] is not None:
                    sp = ann['stop_pos']
                    if plot_start <= sp <= plot_end:
                        ax.plot(sp, y - CDS_HEIGHT / 2 - 0.04, marker='s',
                                markersize=MARKER_SIZE - 1, color='#FF4136',
                                zorder=5, clip_on=True)
                elif ann['is_partial_3']:
                    edge = (exon_list[0][0] if strand == '-'
                            else exon_list[-1][1])
                    if plot_start <= edge <= plot_end:
                        ax.plot(edge, y - CDS_HEIGHT / 2 - 0.04, marker='s',
                                markersize=MARKER_SIZE - 1, color='#FF4136',
                                markerfacecolor='none', markeredgewidth=1.2,
                                zorder=5, clip_on=True)
                ...
                for sj in ann['splice_sites']:
                    if sj['class'] != 'canonical':
                        ...
                        if plot_start <= jx <= plot_end:
                            ax.plot(jx, y, marker='x', markersize=7,
                                    markeredgewidth=2, color='red', zorder=6)

            # ORF label
            if show_orf_labels and ann['orf_label']:
                tx_right = max(e for _, e in exon_list)
                if tx_right <= plot_end:
                    ax.text(tx_right + (plot_end - plot_start) * 0.005, y,
                            ann['orf_label'],
            ...
            if show_frame_qc and not ann['frame_ok']:
                ...
                ax.text(tx_left - (plot_end - plot_start) * 0.01, y,
...
def main():
    ...
    parser.add_argument('--genome', default=None,
                        help='Genome FASTA (required for --infer-cds '
                             'and --show-splice-qc)')
    parser.add_argument('--show-utrs', action='store_true', default=False,
                        help='Render UTR as lighter shade (default: off)')
    parser.add_argument('--show-start-stop', action='store_true', default=False,
                        help='Mark start/stop codon positions (default: off)')
    parser.add_argument('--infer-cds', action='store_true', default=False,
                        help='Infer ORF/CDS for exon-only transcripts '
                             '(default: off)')
    parser.add_argument('--orf-policy',
                        choices=['longest_atg_preferred', 'longest_any'],
                        default='longest_atg_preferred',
                        help='ORF selection policy (default: '
                             'longest_atg_preferred)')
    parser.add_argument('--min-orf-aa', type=int, default=100,
                        help='Minimum ORF length in codons (default: 100)')
    parser.add_argument('--allow-partial-orf', action='store_true',
                        default=True,
                        help='Allow partial ORFs (default: true)')
    parser.add_argument('--show-splice-qc', action='store_true', default=False,
                        help='Highlight non-canonical splice junctions '
                             '(default: off)')
    parser.add_argument('--show-orf-labels', action='store_true', default=False,
                        help='Show compact ORF status labels (default: off)')
    parser.add_argument('--show-frame-qc', action='store_true', default=False,
                        help='Show frame-break warnings (default: off)')
    parser.add_argument('--max-tx-per-track', type=int,
                        default=_DEFAULT_MAX_TX_PER_TRACK,
                        help='Max transcripts per track per locus '
    ...
    for i, (chrom, start, end, dtype, label) in enumerate(disagreements):
        ...
        plot_locus(data, chrom, start, end, f"{dtype} {label}", filename,
                   cds_data=cds_data,
                   genome=genome,
