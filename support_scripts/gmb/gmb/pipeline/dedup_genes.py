...
from gmb.utils.intervals import containment_overlap as _reciprocal_overlap
...
def dedup_genes(
    ...
    for i in range(len(gene_info)):
        ...
        for j in range(i + 1, len(gene_info)):
            ...
            ro = _reciprocal_overlap(gi["start"], gi["end"], gj["start"], gj["end"])
            if ro < threshold:
                continue
            ...
            if gi["chains"] and gj["chains"]:
                structures_match = _chains_match(gi["chains"][0], gj["chains"][0], tolerance)
